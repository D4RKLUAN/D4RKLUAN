<div align="center">

```
██████╗ ██╗  ██╗██████╗ ██╗  ██╗██╗     ██╗   ██╗ █████╗ ███╗   ██╗
██╔══██╗██║  ██║██╔══██╗██║ ██╔╝██║     ██║   ██║██╔══██╗████╗  ██║
██║  ██║███████║██████╔╝█████╔╝ ██║     ██║   ██║███████║██╔██╗ ██║
██║  ██║╚════██║██╔══██╗██╔═██╗ ██║     ██║   ██║██╔══██║██║╚██╗██║
██████╔╝     ██║██║  ██║██║  ██╗███████╗╚██████╔╝██║  ██║██║ ╚████║
╚═════╝      ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═══╝
```

### `whoami`

**Cybersecurity Analyst · Purple Team · Full-Stack Builder**  
Belo Horizonte, MG — Brazil

[![LinkedIn](https://img.shields.io/badge/LinkedIn-luandsr-0a66c2?style=flat-square&logo=linkedin)](https://linkedin.com/in/luandsr)
[![HackTheBox](https://img.shields.io/badge/HackTheBox-D4RKLUAN-9fef00?style=flat-square&logo=hackthebox&logoColor=black)](https://hackthebox.com)
[![Email](https://img.shields.io/badge/Email-luanddlsln075@gmail.com-ea4335?style=flat-square&logo=gmail&logoColor=white)](mailto:luanddlsln075@gmail.com)

</div>

---

## > cat about.txt

```python
class D4RKLUAN:
    role        = "Cybersecurity Analyst @ allu."
    focus       = ["Application Defense", "Incident Response", "Purple Team"]
    currently   = ["Cybersecurity @ FIAP", "HTB & TryHackMe grind", "Building side projects"]
    looking_for = "Opportunities in AppSec · Cloud Security · Incident Response"
    fun_fact    = "I break things to understand how to protect them — then I build them anyway 🛠️"
```

---

## > ls -la skills/

### 🔴 Offensive
![Burp Suite](https://img.shields.io/badge/Burp_Suite-FF6633?style=flat-square&logo=burpsuite&logoColor=white)
![OWASP](https://img.shields.io/badge/OWASP_Top_10-000000?style=flat-square&logo=owasp)
![HackTheBox](https://img.shields.io/badge/HackTheBox-9fef00?style=flat-square&logo=hackthebox&logoColor=black)
![TryHackMe](https://img.shields.io/badge/TryHackMe-212c42?style=flat-square&logo=tryhackme)

### 🔵 Defensive
![Cloudflare](https://img.shields.io/badge/Cloudflare-F38020?style=flat-square&logo=cloudflare&logoColor=white)
![Microsoft Defender](https://img.shields.io/badge/Microsoft_Defender-0078D4?style=flat-square&logo=microsoft&logoColor=white)
![SentinelOne](https://img.shields.io/badge/SentinelOne-7B2ABF?style=flat-square)
![Azure](https://img.shields.io/badge/Azure_Security-0089D6?style=flat-square&logo=microsoft-azure&logoColor=white)

### 🛠️ Dev & Automation
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat-square&logo=go&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)

### 🌐 Web Stack
![Next.js](https://img.shields.io/badge/Next.js_14-000000?style=flat-square&logo=next.js)
![React](https://img.shields.io/badge/React_19-61DAFB?style=flat-square&logo=react&logoColor=black)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_v4-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white)
![Cloudflare Pages](https://img.shields.io/badge/Cloudflare_Pages-F38020?style=flat-square&logo=cloudflare&logoColor=white)

### 🗄️ Databases & Cloud
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![BigQuery](https://img.shields.io/badge/BigQuery-4285F4?style=flat-square&logo=google-cloud&logoColor=white)

---

## > cat projects.md

### 🔒 [Mini-GPO](https://github.com/D4RKLUAN) — Corporate Endpoint Management System
> Go agent + Python/Flask dashboard for remote management of corporate endpoints

- **Stack:** Go 1.21 · Python/Flask · MQTT · GCP · AES-256 · HTML/CSS/JS
- **Highlights:** 17-20MB RAM footprint · <100ms startup · offline queue · multi-platform (Win/Linux/macOS)
- **Security:** AES-256 encryption · bcrypt · CSRF protection · full audit trail

### 🌐 Personal Site / SaaS
> Full-stack PWA with payments, auth, real-time features and maps integration

- **Stack:** Next.js 14 · React 19 · TypeScript · Tailwind v4 · Supabase · Cloudflare Pages
- **Integrations:** Pagar.me · Mercado Pago (subscriptions) · Google Maps API
- **Infra:** OpenNext · Wrangler · pnpm · Row Level Security (RLS)

### 🤖 METRATON AI — Automated Security Recon
> Open-source AI-assisted pentest tool for automated reconnaissance

- Nmap enumeration · HTTP header analysis · technology fingerprinting · DNS/WHOIS collection
- Identified Next.js stack leaks, cache misconfigurations, and expanded attack surface

---

## > tail -f experience.log

```
[2025-07 → now]  IT Support & Security Analyst   @ allu.
[2025-01 → 06]   IT Support Technician           @ TIERh
[2023-02 → 2024-07]  Cybersecurity Analyst       @ 123milhas  ← WAF · Cloudflare · Defender · ThreatX
[2020-08 → 2023-02]  IT Technician               @ 123milhas
```

---

## > ./stats.sh

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=D4RKLUAN&show_icons=true&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=9fef00&icon_color=9fef00&text_color=c9d1d9)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=D4RKLUAN&layout=compact&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=9fef00&text_color=c9d1d9)

</div>

---

## > cat certs.txt

```
✅  Offensive Security Training    — Alura
✅  CTF                            — Hacker Security  
✅  FrontEnd Development           — Descomplica (Nov 2024)
✅  Object Oriented Programming    — Descomplica
🎯  eJPT / OSCP                   — [ in progress ]
📚  Cybersecurity Degree           — FIAP (2026–2027)
```

---

<div align="center">

*"An attacker's mindset. A defender's discipline."*

![Visitor Count](https://komarev.com/ghpvc/?username=D4RKLUAN&color=9fef00&style=flat-square&label=profile+views)

</div>
